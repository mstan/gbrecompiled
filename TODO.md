...
1. Add builtin hardware alias provenance to the metadata sidecar so tooling can distinguish imported names from recompiler-provided constants.
2. Extend symbolized address lifting into more ROM data/table references where the target is known and safe.
3. Export unresolved indirect-jump sites and RAM overlay regions into `*_metadata.json`.
