...
| 2 | Cycle / timing | **sub-instruction access placement done; per-M-cycle timer pending** | `mem_timing` Failed 3→**2** (`modify_timing` PASS via RMW tick-split, interp+emitter; ALU/BIT (HL) reads tick-before'd). `read/write_timing` still FAIL — they measure the read cycle via a **timer-driven** RAM counter, so they need the timer to advance **per-M-cycle within every instruction** (whole-instruction tick batches all increments at once) | Per-M-cycle timer/IRQ advance across all instructions (`gb_tick` in M-cycle steps); ref `gb_timing.h`, opt-out per-region for hot paths |
