...
- [ ] Apply `.sym` names to internal labels, not just function entrypoints
- [ ] Preserve address-based fallback labels when no symbol exists
- [ ] Resolve label-name collisions deterministically
...
- [ ] Load symbol names for WRAM, HRAM, SRAM, and ROM data labels
- [ ] Use symbolic names in generated memory accesses where safe
- [ ] Add hardware/register aliasing on top of raw addresses
...
- [ ] Track whether a name came from `.sym`, builtin hardware aliases, or autogenerated fallback
- [ ] Preserve the original source symbol alongside the emitted sanitized C name
...
- [ ] Emit JSON or YAML metadata next to generated code
- [ ] Include:
...
1. Extend `.sym` support from function names into internal labels.
2. Add a metadata sidecar export for functions, labels, and symbol provenance.
3. Start replacing obvious raw hardware and RAM addresses with symbolic names where safe.
