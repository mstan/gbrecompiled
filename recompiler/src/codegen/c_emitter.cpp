...
GeneratedOutput generate_output(const ir::Program& program,
    ...
    // Forward declarations (emitted to dispatch file for reference;
    // actual prototypes go in the header for cross-bank visibility)
    source_ss << "\n";
    ...
    // Emit each function with real IR code, grouped by bank
    std::map<int, std::ostringstream> bank_streams;

    // Initialize bank stream headers
    auto get_bank_stream = [&](int bank) -> std::ostringstream& {
        if (bank_streams.find(bank) == bank_streams.end()) {
            auto& bs = bank_streams[bank];
            bs << "/* Generated by gbrecomp — Bank " << bank << " */\n";
            bs << "#include \"" << options.output_prefix << ".h\"\n";
            bs << "#include \"gbrt.h\"\n";
            bs << "#include <stdio.h>\n";
            bs << "#include <stdlib.h>\n\n";
        }
        return bank_streams[bank];
    };
    ...
    for (const auto& [name, func] : program.functions) {
        ...
        auto& out = get_bank_stream((int)func.bank);

        out << "/* Function at ";
        if (func.bank > 0) {
            out << std::hex << std::setfill('0') << std::setw(2) << (int)func.bank << ":";
        }
        out << std::hex << std::setfill('0') << std::setw(4) << func.entry_address << std::dec << " */\n";
        out << "void " << func.name << "(GBContext* ctx) {\n";
        ...
        for (uint32_t block_id : sorted_block_ids) {
            ...
            if (it != program.blocks.end()) {
                out << "        case 0x" << std::hex << std::setfill('0') << std::setw(4) 
        ...
        for (size_t block_idx = 0; block_idx < sorted_block_ids.size(); block_idx++) {
            ...
            for (size_t i = 0; i < block.instructions.size(); ++i) {
                ...
                emit_ir_instruction(out, ir_instr, program, 1, options, next_pc, cycles_to_pass, is_last_in_block, func.name, inlineable_functions);
            }
                    ...
                    if (fallthrough_exists_in_function) {
                        ...
                        for (const auto& kv : program.functions) {
                            ...
                            if (target_func.bank == func.bank && target_func.entry_address == fallthrough_addr) {
                                out << "    /* fallthrough to function */\n";
                                
                                if (inlineable_functions.count(target_func.name)) {
                                    // INLINE FALLTHROUGH
                                    out << "    /* Inline: " << target_func.name << " */ {\n";
                                    ...
                                    for (size_t k = 0; k < limit; k++) {
                                        inline_cycles += block.instructions[k].cycles;
                                        emit_ir_instruction(out, block.instructions[k], program, 2, options, 0, 0, false, "", inlineable_functions);
                                    }
                                    if (block.instructions.size() > limit) inline_cycles += block.instructions.back().cycles;
                                    ...
                                    if (options.emit_cycle_counting) {
                                         out << "        gb_tick(ctx, " << (int)inline_cycles << ");\n";
                                         out << "        if (ctx->stopped) return;\n";
                                    }
                                    out << "    } /* End Inline */\n";
                                } else {
                                    out << "    " << target_func.name << "(ctx);\n";
                                }
                                out << "    return;\n";
                        ...
                        if (!found_target_func) {
                            ...
                            out << "    /* warning: fallthrough to unanalyzed code at 0x" 
    ...
    // Collect per-bank source files
    for (auto& [bank, ss] : bank_streams) {
        char fname[64];
        snprintf(fname, sizeof(fname), "%s_bank_%02x.c", options.output_prefix.c_str(), bank);
        output.bank_files[bank] = fname;
