...
If you have a community `.sym` file, you can feed it into the recompiler so emitted functions and internal block labels use readable names instead of only `func_xx_yyyy` and `loc_xx_yyyy` identifiers.
...
Imported names are sanitized into valid C identifiers and emitted with a `sym_` prefix, so labels like `EnterMap`, `DisableLCD`, or `OverworldLoopLessDelay.checkIfStartIsPressed` become safe generated names such as `sym_EnterMap` and `sym_OverworldLoopLessDelay_checkIfStartIsPressed`.

When `--symbols` is enabled, the generated output also includes:

- `*_metadata.json`: a sidecar with emitted function names, block labels, source symbol names, and provenance (`imported` vs `autogenerated`)
- named hardware/RAM constants in the internal header, so immediate accesses become easier to read (for example `GB_IO_LCDC` and `GB_WRAM0_BASE + 0x0f1a`)
