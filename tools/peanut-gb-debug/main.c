 ...
 *
 * Usage:
 *   gb-debug-emu [--port N] [--headless] [--frames N] [--sav file.sav] rom.gb
 */
...
int main(int argc, char *argv[])
{
    ...
    if (!rom_path) {
        fprintf(stderr, "Usage: gb-debug-emu [--port N] [--headless] [--frames N] [--sav file.sav] rom.gb\n");
        return 1;
    }
    ...
    if (sav_path) {
        ...
        if (!sf) {
            fprintf(stderr, "Failed to open sav: %s\n", sav_path);
            return 1;
        }
