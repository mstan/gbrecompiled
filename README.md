...
| `--symbols <file>` | Load a `.sym` symbol file and use those names for generated functions |
...
**Symbol Maps for Better Function Names:**
If you have a community `.sym` file, you can feed it into the recompiler so emitted functions use readable names instead of only `func_xx_yyyy` identifiers.
